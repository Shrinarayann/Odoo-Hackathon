...
def get_user_products():
    ...
    try:
        ...
        status = request.args.get('status')  # 'active', 'sold', or 'all'
        category = request.args.get('category')
        
        # Build query
        query = Product.objects(user_id=user)
        
        # Filter by status
        if status == 'active':
            query = query.filter(is_sold=False)
        elif status == 'sold':
            query = query.filter(is_sold=True)
        ...
        # Apply pagination
        products = query.order_by('-created_at').skip((page - 1) * per_page).limit(per_page)
